...
const weatherService = {
  ...
  /**
   * Get weather prediction based on location and future date (requires authentication)
   */
  getWeatherPrediction: async (locationData) => {
    try {
      const token = localStorage.getItem('token');
      if (!token) {
        throw new Error('Authentication required for weather predictions');
      }

      const response = await axios.post('http://localhost:8080/api/predictions/forecast', locationData, {
        headers: {
      ...
      throw error.response?.data?.message || error.message || 'Failed to get weather prediction';
    }
  }
};
